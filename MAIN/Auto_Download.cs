...
namespace MAIN
{
    class Auto_Download
    {
        ...
        private DataTable SEL_DATA()
        {
            ...
            try
            {
                string process_name = "PROC_GET_INFOR_DOWNLOAD";
            ...
            try
            {
                ...
                for (int i = 0; i < irow; i++)
                {
                    ...
                    try
                    {
                        ...
                        {
                                tiSv = aResponse.LastModified;
                                tiLo = System.IO.File.GetLastWriteTime(_exePath + folder + "\\" + fileName);
                        }
                       
                        if (tiSv != tiLo)
                        {
                            DownloadFTP(ftpDirectory, folder, fileName);
                            File.SetLastWriteTime(_exePath + folder + "\\" + fileName, tiSv);
                        }
                    }
                    catch (Exception ex)
                    {
