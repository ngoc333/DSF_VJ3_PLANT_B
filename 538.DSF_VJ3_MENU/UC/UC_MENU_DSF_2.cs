...
namespace FORM.UC
{
    ...
    {
        public UC_MENU_DSF_2(string Title, string line, string mline, string lang, string ucNumber)
        {
            InitializeComponent();
            lbl_Line.Text = Title;
            ...
            picLine.Name += ucNumber;
            lblLine.Name += ucNumber;
        }
       ...
       // public delegate void (string MenuName, string ButtonCD);
        ...
        public void BindingData(DataTable dt, int i, DataTable dt1)
        {
            try
            {
                ...
                if (dt1 != null)
                {
                    FullText[i] = dt1.Rows[i]["MODEL"].ToString();
                    Init(i);
                }
